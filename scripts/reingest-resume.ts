/**
 * scripts/reingest-resume.ts — Resume ingestion from scraped-data-remaining.json
 *
 * Picks up where reingest-clean.ts left off after an OOM crash at doc 30,450.
 * Reads scraped-data-remaining.json (docs 30,451–50,000) and runs the
 * chunk → embed pipeline WITHOUT clearing existing data.
 *
 * Usage:
 *   NODE_OPTIONS="--max-old-space-size=6144" npx tsx scripts/reingest-resume.ts
 */
...
async function main() {
  ...
  // Load remaining scraped data (docs 30,451+)
  const rawData = fs.readFileSync("scripts/scraped-data-remaining.json", "utf-8");
  const documents: ScrapedDocument[] = JSON.parse(rawData);
  console.log(`Loaded ${documents.length} remaining documents to ingest`);

  console.log("\n--- Ingesting remaining documents ---");
