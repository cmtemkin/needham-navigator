...
- **Tiered search** — search results prioritize Needham-specific content, and if no local results match, the search expands to broader sources. As of v0.19.0 the indexed corpus is **Needham-only**: mass.gov and neighbouring-town pages were removed because they made up 84% of storage without improving answers to Needham questions. Questions about state programs or statewide regulations will return less than they did previously; the search tier router still recognises those queries, so state content can be re-added if it proves needed.
